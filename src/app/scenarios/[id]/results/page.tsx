  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <Link href="/" className="inline-flex items-center gap-1 text-sm text-violet-600 hover:text-violet-800 transition-colors mb-1.5">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
            </svg>
            Все сценарии
          </Link>
          <h1 className="text-2xl font-bold text-slate-900">Результаты запусков</h1>
          {results.length > 0 && (
            <p className="text-sm text-slate-500 mt-0.5">{results.length} {results.length === 1 ? 'запуск' : results.length < 5 ? 'запуска' : 'запусков'}</p>
          )}
        </div>
        <button
          onClick={handleRun}
      ...
      {results.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-slate-500 text-sm">Нет запусков. Нажмите «Запустить снова» чтобы начать тест.</p>
        </div>
      ) : (
